...
module.exports = {
    updateList: function(entry){
        let RecentRedeems = fs.readFileSync(`${process.cwd()}/settings/redeemHistory.txt`).toString().split("\n");

        //If the array of the file isn't the size of target size, reset it
        if(RecentRedeems.length != TargetSize+1) RecentRedeems = ["Empty for now!", "Empty for now!", "Empty for now!"];
        ...
        //Write the file
        fs.writeFileSync(`${process.cwd()}/settings/redeemHistory.txt`, ExportString);
    }
}