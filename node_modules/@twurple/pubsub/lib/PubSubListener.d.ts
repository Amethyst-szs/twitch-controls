import type { PubSubMessage } from './messages/PubSubMessage';
import type { SingleUserPubSubClient } from './SingleUserPubSubClient';
/**
 * A listener attached to a single PubSub topic.
 */
export declare class PubSubListener<T extends PubSubMessage = PubSubMessage> {
    private readonly _topic;
    private readonly _userId;
    private readonly _callback;
    private readonly _client;
    /** @private */
    constructor(_topic: string, _userId: string, _callback: (message: T) => void, client: SingleUserPubSubClient);
    /**
     * The type of the topic.
     */
    get topic(): string;
    /**
     * The user ID part of the topic.
     */
    get userId(): string;
    /**
     * Removes the topic from the PubSub client.
     */
    remove(): Promise<void>;
    /** @private */
    call(message: T): void;
}
//# sourceMappingURL=PubSubListener.d.ts.map