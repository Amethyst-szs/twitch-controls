...
const HostTarget_1 = require("./caps/twitchCommands/messageTypes/HostTarget");
 ...
 * @hideProtected
 */
let ChatClient = ChatClient_1 = class ChatClient extends ircv3_1.IrcClient {
    /**
    ...
    constructor(config = {}) {
         ...
         */
        this.onHostsRemaining = this.registerEvent();
        /**
        ...
        this._onBanResult = this.registerInternalEvent();
        this._onTimeoutResult = this.registerInternalEvent();
        this._onUnbanResult = this.registerInternalEvent();
        this._onColorResult = this.registerInternalEvent();
        this._onCommercialResult = this.registerInternalEvent();
        this._onDeleteMessageResult = this.registerInternalEvent();
        this._onEmoteOnlyResult = this.registerInternalEvent();
        this._onEmoteOnlyOffResult = this.registerInternalEvent();
        this._onFollowersOnlyResult = this.registerInternalEvent();
        this._onFollowersOnlyOffResult = this.registerInternalEvent();
        this._onHostResult = this.registerInternalEvent();
        this._onUnhostResult = this.registerInternalEvent();
        this._onModResult = this.registerInternalEvent();
        this._onUnmodResult = this.registerInternalEvent();
        this._onModsResult = this.registerInternalEvent();
        this._onJoinResult = this.registerInternalEvent();
        this._onR9kResult = this.registerInternalEvent();
        this._onR9kOffResult = this.registerInternalEvent();
        this._onSlowResult = this.registerInternalEvent();
        this._onSlowOffResult = this.registerInternalEvent();
        this._onSubsOnlyResult = this.registerInternalEvent();
        this._onSubsOnlyOffResult = this.registerInternalEvent();
        this._onVipResult = this.registerInternalEvent();
        this._onUnvipResult = this.registerInternalEvent();
        this._onVipsResult = this.registerInternalEvent();
        if (((_e = config.authProvider) === null || _e === void 0 ? void 0 : _e.tokenType) === 'app') {
        ...
        this._chatLogger = logger_1.createLogger({
        ...
        const executeChatMessageRequest = async ({ type, message, channel, tags }) => {
            if (type === 'say') {
        ...
        const executeWhisperRequest = async ({ target, message }) => this._doWhisper(target, message);
        ...
        this._whisperRateLimiter = new rate_limiter_1.TimedPassthruRateLimiter(new rate_limiter_1.TimeBasedRateLimiter({
            bucketSize: whisperLimitPerSecond,
            timeFrame: 1200,
            logger: { minLevel: logger_1.LogLevel.ERROR },
            doRequest: executeWhisperRequest
        }), { bucketSize: whisperLimitPerMinute, timeFrame: 64000 });
        this.addCapability(twitchTags_1.TwitchTagsCapability);
        this.addCapability(twitchCommands_1.TwitchCommandsCapability);
        if (config.requestMembershipEvents) {
            this.addCapability(TwitchMembershipCapability_1.TwitchMembershipCapability);
        }
        this.addInternalListener(this.onRegister, async () => {
            this._authVerified = true;
            const resolvedChannels = await shared_utils_1.resolveConfigValue(config.channels);
            ...
            if (resolvedChannels) {
                await Promise.all(resolvedChannels.map(async (channel) => {
                    ...
                    try {
                        ...
                        `Failed to join configured channel ${channel}; original message: ${(_b = (_a = e) === null || _a === void 0 ? void 0 : _a.message) !== null && _b !== void 0 ? _b : e}`);
                    }
                }));
            }
        });
        this.addInternalListener(this.onPrivmsg, (channel, user, message, msg) => {
            if (user === 'jtv') {
                ...
                const match = ChatClient_1.HOST_MESSAGE_REGEX.exec(message);
                if (match) {
        ...
        this.onTypedMessage(ClearChat_1.ClearChat, ({ params: { channel, user }, tags }) => {
            if (user) {
        ...
        this.onTypedMessage(ClearMsg_1.ClearMsg, msg => {
            const { params: { channel }, targetMessageId } = msg;
            this.emit(this.onMessageRemove, channel, targetMessageId, msg);
        });
        this.onTypedMessage(HostTarget_1.HostTarget, ({ params: { channel, targetAndViewers } }) => {
        ...
        this.onTypedMessage(ircv3_1.MessageTypes.Commands.ChannelJoin, ({ prefix, params: { channel } }) => {
            this.emit(this.onJoin, channel, prefix.nick);
        });
        this.onTypedMessage(ircv3_1.MessageTypes.Commands.ChannelPart, ({ prefix, params: { channel } }) => {
            this.emit(this.onPart, channel, prefix.nick);
        });
        this.onTypedMessage(RoomState_1.RoomState, ({ params: { channel }, tags }) => {
            let isInitial = false;
        ...
        this.onTypedMessage(UserNotice_1.UserNotice, userNotice => {
        ...
        this.onTypedMessage(Whisper_1.Whisper, whisper => {
            this.emit(this.onWhisper, whisper.prefix.nick, whisper.params.message, whisper);
        });
        this.onTypedMessage(ircv3_1.MessageTypes.Commands.Notice, async ({ params: { target: channel, content }, tags }) => {
            const messageType = tags.get('msg-id');
            ...
            switch (messageType) {
                ...
                case 'bad_ban_broadcaster': {
                    this.emit(this._onBanResult, channel, userUtil_1.toUserName(channel), messageType);
                    break;
                }
                case 'bad_ban_admin':
                ...
                case 'bad_timeout_broadcaster': {
                    this.emit(this._onTimeoutResult, channel, userUtil_1.toUserName(channel), undefined, messageType);
                    break;
                }
                case 'bad_timeout_mod': {
                    const match = /^You cannot timeout moderator (\w+) unless/.exec(content);
                    if (match) {
                        this.emit(this._onTimeoutResult, channel, userUtil_1.toUserName(match[1]), undefined, messageType);
                    }
                    break;
                }
     ...
     */
    async host(channel, target) {
        const channelName = userUtil_1.toUserName(channel !== null && channel !== void 0 ? channel : this._credentials.nick);
        await new Promise((resolve, reject) => {
            const e = this._onHostResult((chan, error) => {
                if (userUtil_1.toUserName(chan) === channelName) {
                    if (error) {
                        reject(error);
                    }
                    else {
     ...
     */
    async unhost(channel = this._credentials.nick) {
        channel = userUtil_1.toUserName(channel);
        await new Promise((resolve, reject) => {
            const e = this._onUnhostResult((chan, error) => {
                if (userUtil_1.toUserName(chan) === channel) {
                    if (error) {
                        reject(error);
                    }
                    else {
     ...
     * @param channel The channel to end the host on. Defaults to the channel of the connected user.
     */
    async unhostOutside(channel = this._credentials.nick) {
     ...
     */
    async raid(channel, target) {
        target = userUtil_1.toUserName(target);
        await this.say(channel !== null && channel !== void 0 ? channel : this._credentials.nick, `/raid ${target}`);
    }
    /**
     ...
     */
    async unraid(channel) {
        await this.say(channel !== null && channel !== void 0 ? channel : this._credentials.nick, '/unraid');
    }
    /**
     ...
     */
    async ban(channel, user, reason = '') {
        const channelName = userUtil_1.toUserName(channel !== null && channel !== void 0 ? channel : this._credentials.nick);
        user = userUtil_1.toUserName(user);
        await new Promise((resolve, reject) => {
            const e = this._onBanResult((_channel, _user, error) => {
                if (userUtil_1.toUserName(_channel) === channelName && userUtil_1.toUserName(_user) === user) {
                    if (error) {
                        reject(error);
                    }
                    else {
     ...
     * @param channel The channel to ban the user from. Defaults to the channel of the connected user.
     */
    async clear(channel = this._credentials.nick) {
        channel = userUtil_1.toUserName(channel);
        await new Promise(resolve => {
            const e = this.addInternalListener(this.onChatClear, _channel => {
                if (userUtil_1.toUserName(_channel) === channel) {
                    resolve();
                    e.unbind();
                }
            });
     ...
     */
    async runCommercial(channel, duration) {
        channel = userUtil_1.toUserName(channel);
        await new Promise((resolve, reject) => {
    ...
    async deleteMessage(channel, message) {
        channel = userUtil_1.toUserName(channel);
        const messageId = ChatMessageAttributes_1.extractMessageId(message);
        await new Promise((resolve, reject) => {
     ...
     */
    async disableEmoteOnly(channel) {
        channel = userUtil_1.toUserName(channel);
        await new Promise((resolve, reject) => {
     ...
     */
    async enableFollowersOnly(channel, minFollowTime = 0) {
        ...
        await new Promise((resolve, reject) => {
            const e = this._onFollowersOnlyResult((_channel, _minFollowTime, error) => {
                if (userUtil_1.toUserName(_channel) === channel && _minFollowTime === minFollowTime) {
                    if (error) {
                        reject(error);
                    }
                    else {
     ...
     */
    async disableFollowersOnly(channel) {
        channel = userUtil_1.toUserName(channel);
        await new Promise((resolve, reject) => {
     ...
     */
    async mod(channel, user) {
        channel = userUtil_1.toUserName(channel);
        user = userUtil_1.toUserName(user);
        await new Promise((resolve, reject) => {
            const e = this._onModResult((_channel, _user, error) => {
                if (userUtil_1.toUserName(_channel) === channel && userUtil_1.toUserName(_user) === user) {
                    if (error) {
                        reject(error);
                    }
                    else {
     ...
     */
    async enableSlow(channel, delayBetweenMessages = 30) {
     ...
     * @param reason
     */
    async timeout(channel, user, duration = 60, reason = '') {
     ...
     */
    async purge(channel, user, reason = '') {
     ...
     */
    async addVip(channel, user) {
        channel = userUtil_1.toUserName(channel);
        user = userUtil_1.toUserName(user);
        await new Promise((resolve, reject) => {
            const e = this._onVipResult((_channel, _user, error) => {
                if (userUtil_1.toUserName(_channel) === channel && (_user === undefined || userUtil_1.toUserName(_user) === user)) {
                    if (error) {
                        reject(error);
                    }
                    else {
     ...
     */
    async say(channel, message, attributes = {}) {
        const tags = {};
        ...
        await this._messageRateLimiter.request({
            type: 'say',
            channel: userUtil_1.toChannelName(channel),
            message,
            tags
        });
    }
...
Please note that your whispers might not arrive reliably if your bot is not a known or verified bot.`);
        ...
        await this._whisperRateLimiter.request({
            target: userUtil_1.toUserName(user),
            message
        });
    }
    /**
     ...
     */
    async join(channel) {
        await this._joinRateLimiter.request(userUtil_1.toChannelName(channel));
    }
    /**
     ...
     */
    part(channel) {
        super.part(userUtil_1.toChannelName(channel));
    }
    /**
    ...
    async getPassword() {
        ...
        if (this._authToken && !auth_1.accessTokenIsExpired(this._authToken) && this._authVerified) {
        ...
        try {
            ...
            if (this._authToken) {
                const token = await auth_1.getTokenInfo(this._authToken.accessToken);
                if (!token.userName) {
        ...
        try {
            this._authToken = await ((_b = (_a = this._authProvider).refresh) === null || _b === void 0 ? void 0 : _b.call(_a));
            if (this._authToken) {
                const token = await auth_1.getTokenInfo(this._authToken.accessToken);
                if (!token.userName) {
                    throw new auth_1.InvalidTokenTypeError('Could not determine a user name for your token; you might be trying to disguise an app token as a user token.');
    ...
    static *_getReauthenticateWaitTime() {
        let current = 0;
        let next = 1;
        while (current < 120) {
            yield current;
